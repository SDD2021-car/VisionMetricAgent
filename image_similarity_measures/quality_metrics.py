"""
This module is a collection of metrics to assess the similarity between two HR.
...
def _similarity_measure(x: np.array, y: np.array, constant: float):
    """
    Calculate feature similarity measurement between two HR
    """
...
def fsim(
    ...
    Note that FSIM is defined in the original papers for grayscale as well as for RGB HR. Our use cases
    are mostly multi-band HR e.g. RGB + NIR. To accommodate for this fact, we compute FSIM for each individual
    ...
    for i in range(org_img.shape[2]):
        # Calculate the PC for original and predicted HR
        pc1_2dim = pc(
        ...
        # Calculate GM for original and predicted HR based on Scharr operator

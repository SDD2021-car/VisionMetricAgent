...
def cal_batch_SSIM(real_paths, fake_paths):
    ...
    for i in tqdm(range((len(real_paths)))):
        ...
        img1 = Image.open(real_paths[i]).resize((256, 256))
        img2 = Image.open(fake_paths[i]).resize((256, 256))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fake_dir', type=str, default='G:\\eval\\Parallel_GAN\\trans0307\\test_latest\\HR')
    parser.add_argument('--real_dir', type=str, default='G:\\eval\\Parallel_GAN\\trans0307\\test_latest\\GT')
    ...
    real_paths, fake_paths = get_dirImage(opt.real_dir, opt.fake_dir)

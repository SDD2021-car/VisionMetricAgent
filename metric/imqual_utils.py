...
def getSSIM(X, Y):
    """
       Computes the mean structural similarity between two HR.
    """
    # assert (X.shape[0] == Y.shape[0]), "Image-patche provided have different dimensions"
    assert (X.shape[0] == Y.shape[0] & X.shape[1] == Y.shape[1]), "Image-patche provided have different dimensions"
...
def compute_ssim(X, Y):
    """
       Compute the structural similarity per single channel (given two HR)
    """
...
def getFSIM(X,Y):
    target_data = np.array(X, dtype=np.float64)
    ref_data = np.array(Y, dtype=np.float64)
    fsim_result = fsim(target_data,ref_data)
    return fsim_result
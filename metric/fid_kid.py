...
parser.add_argument('--path', nargs=2,default=['/NAS_data/yjy/DDBM_S2O_canny_results/S2O_scene_v2/testB',
                                     '/NAS_data/yjy/DDBM_S2O_canny_results/S2O_scene_v2/result_200000'],
                    help=('Path to the generated HR or ''to .npz statistic files'))
...
parser.add_argument('-c', '--gpu', default='1,2,3,4', type=str,
...
def get_activations(files, model, batch_size=50, dims=2048,
    ...
    """Calculates the activations of the pool_3 layer for all HR.
    Params:
    -- files       : List of image files paths
    -- model       : Instance of inception model
    -- batch_size  : Batch size of HR for the model to process at once.
    ...
    Returns:
    -- A numpy array of dimension (num HR, dims) that contains the
    ...
    for i in tqdm(range(0, len(files), batch_size)):
        ...
        images = []
        for f in files[start:end]:
            img = imread(str(f)).astype(np.float32)
            if img.ndim == 2:  # 灰度图，H x W
                img = np.stack([img] * 3, axis=-1)  # 复制成 H x W x 3
            elif img.shape[2] == 4:  # RGBA 图
...
def calculate_activation_statistics(files, model, batch_size=50,
    ...
    -- files       : List of image files paths
    -- model       : Instance of inception model
    -- batch_size  : The HR numpy array is split into batches with
...
def calculate_given_paths(paths, batch_size, cuda, dims):
    ...
    # model.load_state_dict(torch.load('/data/yjy_data/eval/inceptionv3_encoder36.pth'),  #若测试需要更改权重名称可以更改
    #                       strict=True)
    if cuda:
        model.cuda()
